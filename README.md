# Travel & Carbon Management System

本專題為一套差旅與碳排管理系統，主要解決員工在申報差旅與自用車里程補助時，需查詢地圖並手動計算金額的問題。

系統將距離查詢、補助計算與申請流程整合於同一平台，使用者輸入地址後即可自動完成距離與補助計算，降低人工錯誤，並補充碳排資訊供查詢與分析。

---

## Demo 影片

完整版 Demo（功能完整展示）  
https://youtu.be/Q1Pq8a48sh0  

包含申請流程、距離計算、補助自動帶入與審核機制

---

## 核心功能

- 差旅報支管理（DailyTrip + Application）
- 地圖距離計算（整合 OpenStreetMap / OSRM）
- 自用車補助自動計算
- 主管審核與駁回流程
- 駁回時保留當下資料（快照）
- 費率與碳排係數版本管理
- 差旅費用與碳排分析

---

## 系統設計重點

- 將原本以月為單位的報支流程，調整為「先建立差旅明細，再組成申請單」
- 由 `Application` 統一管理流程狀態，讓狀態判斷集中處理
- 駁回時保存資料快照，避免後續修改影響審核結果
- 制度資料採版本管理，避免新設定影響既有資料
- 將 Controller、Service 與資料存取分開處理
- 距離計算功能獨立整理為服務，與外部地圖服務分開處理

---

## 實作說明

- 使用 OpenStreetMap 取得地址資訊，並透過 OSRM API 計算距離
- 補助金額依里程數與費率於後端統一計算
- 申請駁回時保存當下送審資料，供後續檢視
- 使用 AJAX / fetch 處理部分互動與資料更新
- 分析功能主要針對差旅費用與碳排資料進行統計

---

## 開發過程中的調整

- 初期規劃為「月結式報支」，實作後改為「差旅明細 + 申請單」，提升使用彈性
- 駁回流程初期因多個狀態來源造成判斷複雜，後續改由 `Application` 統一管理流程狀態
- 制度資料若直接修改會影響歷史資料，因此改採版本管理方式處理

---

## 技術

- 後端：ASP.NET Core MVC / C#
- ORM：Entity Framework Core
- 資料庫：SQL Server
- 前端：Razor Views / JavaScript / Bootstrap
- 驗證：ASP.NET Core Identity
- 地圖服務：OpenStreetMap / OSRM
- 圖表：Chart.js

---

## 專案說明

本專案僅作為作品集與功能展示用途。  
未使用任何正式環境的敏感資料，所有資料皆為測試用途。

---

## 測試帳號

本系統提供測試帳號，方便快速體驗主要功能：

- Manager（主管）
- Employee（員工）

---

## 專題文件

完整設計與演進過程請參考：  
- [初始設計與系統演進](./docs/Initial_Idea_from_Excel.md)